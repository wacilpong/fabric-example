/**
 * 
 * @author Jim Ma (https://github.com/mazong1123) Contact: mazong1123@gmail.com
 *         License: MIT
 */
(function() {
	'use strict';
					...
					/** @lends fabric.Canvas */
					{
						...
						_onDoubleClick : function(e) {
							...
							if (target && !self.isDrawingMode) {
								// To unify the behavior, the object's double
								// click event does not fire on drawing mode.
						...
						_onTapHold : function(e) {
							...
							if (target && !self.isDrawingMode) {
								// To unify the behavior, the object's tap hold
								// event does not fire on drawing mode.
								target.fire('taphold', {
									e : e
								});
							}
						...
						_onMouseDown : function(e) {
							...
							// Add right click support and group object click
							// support.
							if (e.which === 3
								...
								if (!isTargetGroup
										|| !self.fireEventForObjectInsideGroup) {
									// Canvas event only for right click. For
									// group object, the super method already
									// fired a canvas event.
								...
								if (target && !self.isDrawingMode) {
									// To unify the behavior, the object's mouse
									// down event does not fire on drawing mode.
						...
						_onDoubleTap : function(e) {
							...
							if (target && !self.isDrawingMode) {
								// To unify the behavior, the object's double
								// tap event does not fire on drawing mode.
						...
						_onLongTapEnd : function(e) {
							...
							if (target && !self.isDrawingMode) {
								// To unify the behavior, the object's long tap
								// end event does not fire on drawing mode.
						...
						_initEventListeners : function() {
							...
							addListener(self.upperCanvasEl, 'dblclick',
									self._onDoubleClick);
						},
						...
						_containsPointForGroupObject : function(pointer, target) {
							...
							return (target.containsPoint(xy) || target
									._findTargetCorner(pointer));
						},

						_adjustPointerAccordingToGroupObjects : function(
								originalPointer, group) {
						...
						findRealTarget : function(e) {
							...
							if (!self.fireEventForObjectInsideGroup) {
								...
								if (target !== undefined
									...
									pointer = self
											._adjustPointerAccordingToGroupObjects(
													pointer, target);
									var i = objects.length;
									while (i--) {
										if (self._checkTargetForGroupObject(
												objects[i], pointer)) {
											target = objects[i];
						...
						removeListeners : function() {
							...
							removeListener(self.upperCanvasEl, 'dblclick',
									self._onDoubleClick);
						},
